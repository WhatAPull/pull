-- No reader holds more than three jobs' worth of the day, and a committed day says so.
--
-- 20260926200000 made the door count the jobs readers have asked for and the worker has not
-- started. It counted every one of them in full, so one account could close the door on
-- everybody for nothing. Eleven submits with no text and no URL, each of which fails at
-- `resolve_identity` for 0 cents when it runs, sat in the stagger for five to forty minutes
-- at 17 cents apiece: `spend_today()` was 0, `generation_waiting_cents()` was 187, every
-- other reader was refused with "the daily generation budget is spent. Summaries resume at
-- 00:00 UTC." and `generation_budget_state()` told everyone `spent`. Resubmitting up to the
-- ceiling of fifty kept it shut for hours, and it cost nothing.
--
-- Two changes:
--
-- 1. One reader's waiting summaries count for at most three jobs' worth:
--    `least(n, 3) * min_job_cents()` per requester. Three is the fast allowance, the jobs a
--    reader can have running at once before the stagger spaces the rest out. This mirrors
--    the study share: a reader's waiting courses already count for no more than their
--    share can still fund. A reader's jobs past the third are not counted and they are not
--    refused either. Refusing a reader for having jobs waiting would break the documented
--    shape of the quota (three fast, then a widening stagger, fifty in total), and those
--    jobs wait their turn on their own. `reserve_budget` still bounds the day, so what this
--    gives up is exactness in the estimate and not the cap: a reader with many jobs queued
--    can use more of the day than the door counted for them, and a later reader's job then
--    waits for room.
--
-- 2. A day that is COMMITTED is not a day that is SPENT. When spend alone leaves room and
--    only the waiting jobs close the door, the refusal says so, with DETAIL `committed` and
--    without promising midnight, since the room comes back as those jobs run.
--    `generation_budget_state()` answers `committed` for that state. The Studio reads it,
--    and asks again while it lasts.
--
-- Everything else is as 20260926200000 left it: readers' jobs only, the rules for a start,
-- the floors, the per-requester lock, and the replay answered before the day is asked.
--
 ...
 *
 * As 20260926200000 states it, with the summary term capped per reader. A job is waiting
 * while it is queued or running and has a requester, nothing charged to it today
 * (`cost_cents > 0`) and nothing held for it today. For each reader:
 *
 *   summaries  least(count, 3) x min_job_cents()
 *   courses    least(count x study_min_job_cents(), what their study share can still fund)
 *
 * Catalogue jobs, which have no requester, are not counted. The operator schedules them,
 * and `reserve_budget` bounds the day whoever holds the money.
 *
 * `security definer` like `spend_today()`, and granted like it: `service_role` only.
 */
...
as $$
  select coalesce(sum(
           case
             ...
             -- Three: the fast allowance in enqueue_generation_job. See the header.
             else least(w.jobs, 3) * public.min_job_cents()
           end), 0)
  from (
...
comment on function public.generation_waiting_cents() is
  ...
  'nothing held today. Each reader''s summaries count at min_job_cents() for at most three '
  'of them, and their study courses at study_min_job_cents() up to what their study share '
  'can still fund. The door in enqueue_generation_job and generation_budget_state() add it '
  'to spend_today(). See 20260926200000 and 20260926210000.';
...
/*
 * `generation_budget_state`, with a fourth answer.
 *
 *   spent      spend alone leaves no room for a job: the door's first refusal, until 00:00 UTC
 *   committed  spend leaves room, and the jobs already waiting take it: the door's second
 *              refusal, until some of them have run
 *   low        four fifths of the day is spent or committed
 *   open       otherwise
 *
 * Still only a word, never a figure (20260914030000). A client that does not know
 * `committed` treats an unknown answer as `open` (`fetchBudgetState`), and the door then
 * refuses with its own sentence, which is where an older bundle already was.
 */
...
declare
  ...
  waiting numeric := public.generation_waiting_cents();
  cap     numeric := public.daily_spend_cap_cents();
begin
  -- The DOOR's two tests, in the door's order.
  if spent + public.min_job_cents() > cap then return 'spent'; end if;
  if spent + waiting + public.min_job_cents() > cap then return 'committed'; end if;
  if spent + waiting >= cap * 0.8 then return 'low'; end if;
  return 'open';
end;
$$;
...
comment on function public.generation_budget_state() is
  ...
  'two refusals of the door in enqueue_generation_job, in its order, so a reader is never '
  'shown room that does not exist. Never the figures themselves: see 20260914030000.';
...
/*
 * `enqueue_generation_job`, restated from 20260926200000. The only change is in the section
 * headed "REFUSED WHERE A JOB COULD NOT RUN": the budget test is split into its two
 * refusals, spent and committed. Every other line is as that migration left it.
 */
...
begin
   ...
   * jobs' worth for any one reader (20260926210000). The rules for what counts are stated
   * there.
   *
